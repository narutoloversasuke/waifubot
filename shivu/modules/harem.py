...
async def mywaifus(update: Update, context: CallbackContext, page=0) -> None:
    ...
    harem_message = f"<b>{escape(update.effective_user.first_name)}'s Waifu Collection - Page {page+1}/{total_pages}</b>\n"
...
async def mywaifus_callback(update: Update, context: CallbackContext) -> None:
    ...
    if query.from_user.id != user_id:
        await query.answer("It's Not Your Collection", show_alert=True)
        return
