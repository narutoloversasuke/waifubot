...
db.user_characters.create_index([('characters.vid_url', ASCENDING)])
...
async def inlinequery(update: Update, context: CallbackContext) -> None:
    ...
    if query.startswith('collection.'):
        ...
        if user_id.isdigit():
            ...
            if user:
                ...
                if search_terms:
                    ...
                    all_characters = [character for character in all_characters if regex.search(character['name'])
                                     ]
            else:
        ...
        if query:
            ...
            all_characters = list(await collection.find({"$or": [{"name": regex}]}).to_list(length=None))
        else:
        ...
        if 'img_url' in character:
            ...
            results.append(
                InlineQueryResultVideo(
                    thumbnail_url=character['vid_url'],
                    id=f"{character['id']}_{time.time()}",
